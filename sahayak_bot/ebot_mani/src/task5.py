...
CODE DESCRIPTION: -> This code is responsible for completing task 5, ie. moving a UR5 robotic arm using Moveit to pick and place 3 objects(coke, battery, glue)
...
ORDER IN WHICH OBJECTS ARE PICKED - 1. FPGA Board from store room
									2. Coke from Pantry
									3. Glue from Meeting Room
...
# Return Value 	- None
# Logic 		- Pases the given coordinates to MoveBase as goal, and thus navigates to the given coordinates
...
def pick_object(object,gripper_v,coke_closure=0):
	...
	if (object == 'Glue'): 
		...
		#print colored("Y value: ", point_target.point.y)

		if (point_target.point.y < -0.2):
			pose_target.pose.position.x = point_target.point.x - 0.2
			pose_target.pose.position.y = point_target.point.y - 0.009
...
def place_object(object):
	...
	arm_group.set_named_target(object+"_drop_pose")
	arm_group.go(wait=True)
...
if __name__ == '__main__':
	...
	Dict = {'Store_room':{'location':[[25.900, -3.0452, 0, 0, -0.66],[24.985, -4.123, 0, 0, -2.763030]]}, 
			...
			'Research_lab':{'location':[[10.8279, 9.8527, 0, 0, -1.5707]], 'pose':[0.38526, 0.51656, 1.02126, 0.196038,-0.67939,-0.66485,0.240738]}, 
			...
			'Home':{'location':[[0,0,0,0,0]]}}
	
	#Dictionary having the values of gripper closing for various objects
	Object_dict = {'FPGA_Board': 0.250,'Glue':0.230,'Coke': 0.230}

	#Pick up order: FPGA, coke, glue
	# Store_room, conference room(drop FPGA board), pantry (get coke), meeting room (drop the coke & pick up the glue), research lab (drop glue) 
	print colored("Started Run!", "blue")
	
	
	#Navigating to store room to pick FPGA Board
	...
	#Picking up FPGA Board
	pick_object('FPGA_Board',Object_dict['FPGA_Board'])

	print colored("FPGA Board Picked", "blue")
	...
	#Navigating to Conference room to place FPGA Board
	...
	print colored("Conference Room Reached", "blue")

	#Placing the FPGA Board
	place_object('FPGA_Board')
	
	print colored("FPGA Board Dropped in DropBox1", "blue")
	...
	#Navigating out of pantry and to meeting room to place cooke can and pick glue
	...
	navigate(Dict['Meeting_room']['location'][0][0], Dict['Meeting_room']['location'][0][1], Dict['Meeting_room']['location'][0][2],Dict['Meeting_room']['location'][0][3], Dict['Meeting_room']['location'][0][4])
	
	print colored("Meeting Room Reached", "blue")
	
	navigate(Dict['Meeting_room']['location'][1][0], Dict['Meeting_room']['location'][1][1], Dict['Meeting_room']['location'][1][2],Dict['Meeting_room']['location'][1][3], Dict['Meeting_room']['location'][1][4])
	
	#Placing Coke Can
	place_object('Coke')
	
	print colored("Coke Dropped in DropBox2", "blue")

	
	#Picking up Glue
	pick_object('Glue',Object_dict['Glue'])
	#place_object('Glue')
	
	print colored("Glue Picked", "blue")
	
	#Navigating out from meeting room to research lab to place glue
	navigate(Dict['Meeting_room']['location'][2][0], Dict['Meeting_room']['location'][2][1], Dict['Meeting_room']['location'][2][2],Dict['Meeting_room']['location'][2][3], Dict['Meeting_room']['location'][2][4])
	...
	print colored("Glue Dropped in DropBox3", "blue")
