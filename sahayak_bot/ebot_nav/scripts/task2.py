...
import rospy
import time
# Importing actionlib to create move base action client 
import actionlib
# Importing smach libraries for state machines
import smach
import smach_ros
...
class Navigate(smach.State):
	# Init function
	# Initializes the states and navigation goal co-ordinates
	def __init__(self,nav_x,nav_y):
		smach.State.__init__(self, outcomes=['Goal_reached', 'Goal_cancelled', 'Goal_aborted'])
		self.nav_x=nav_x
		self.nav_y=nav_y	

	# Function defining active state
	def active_cb(self):
		rospy.loginfo("Going to goal")

	# Function defining completed state
	def done_cb(self,status, result):
		self.stat=status
		return self.stat

	# Execute Function
	# Make the robot go towards the way-point
	def execute(self, userdata):
		...
		# Keeping the orientation same as that of the map
		...
		# Checking if task is finished or not		
		if not finished:
